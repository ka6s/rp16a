...
#define ADDR_I2C2SPI 0x24 /* Alex I2C to SPI */
...
void alex_write()
{
  ...
  if(alex_data_rx != data) {
    ...
    fprintf(stderr,"RX:");
    if((data & 0x0001) > 0) { fprintf(stderr,"-Yellow-");}
    if((data & 0x0002) > 0) { fprintf(stderr,"-13MHPF-");}
    if((data & 0x0004) > 0) { fprintf(stderr,"-20MHPF-");}
    if((data & 0x0008) > 0) { fprintf(stderr,"-6MPre-");}
    if((data & 0x0010) > 0) { fprintf(stderr,"-9.5HPF-");}
    if((data & 0x0020) > 0) { fprintf(stderr,"-6.5HPF-");}
    if((data & 0x0040) > 0) { fprintf(stderr,"-1.5HPF-");}
    if((data & 0x0080) > 0) { fprintf(stderr,"-N/C-");}
    if((data & 0x0100) > 0) { fprintf(stderr,"-XVTIn-");}
    if((data & 0x0200) > 0) { fprintf(stderr,"-RX2In-");}
    if((data & 0x0400) > 0) { fprintf(stderr,"-RX1In-");}
    if((data & 0x0800) > 0) { fprintf(stderr,"-RX1Out-");}
    if((data & 0x1000) > 0) { fprintf(stderr,"-BYP-");}
    if((data & 0x2000) > 0) { fprintf(stderr,"-20db-");}
    if((data & 0x4000) > 0) { fprintf(stderr,"-10db-");}
    if((data & 0x8000) > 0) { fprintf(stderr,"-Red--");}
    fprintf(stderr,"::%4x\n",data);
    ...
    // KA6S
       ...
       if((i2c_resp=ioctl(i2c_fd, I2C_SLAVE_FORCE, ADDR_I2C2SPI)) >= 0) {
          i2c_write_addr_data16(i2c_fd, 1, alex_data_rx);
       }
    // End of KA6S add
    }
  }
  ...
  if(alex_data_tx != data)
  {
    #ifdef DEBUG
    fprintf(stderr,"TX:");
    if((data &0x0001) > 0) { fprintf(stderr,"-N/C-");}
    if((data &0x0002) > 0) { fprintf(stderr,"-N/C-");}
    if((data &0x0004) > 0) { fprintf(stderr,"-N/C-");}
    if((data &0x0008) > 0) { fprintf(stderr,"-Yellow-");}
    if((data &0x0010) > 0) { fprintf(stderr,"-30/20MF-");}
    if((data &0x0020) > 0) { fprintf(stderr,"-60/40M-");}
    if((data &0x0040) > 0) { fprintf(stderr,"-80M-");}
    if((data &0x0080) > 0) { fprintf(stderr,"-160M-");}
    if((data &0x0100) > 0) { fprintf(stderr,"-ANT1-");}
    if((data &0x0200) > 0) { fprintf(stderr,"-ANT2-");}
    if((data &0x0400) > 0) { fprintf(stderr,"-ANT3-");}
    if((data &0x0800) > 0) { fprintf(stderr,"-T/R-");}
    if((data &0x1000) > 0) { fprintf(stderr,"-RED-");}
    if((data &0x2000) > 0) { fprintf(stderr,"-6MBYPb-");}
    if((data &0x4000) > 0) { fprintf(stderr,"-12/10M-");}
    if((data &0x8000) > 0) { fprintf(stderr,"-17/15M--");}
    fprintf(stderr,"::%4x\n",data);
    ...
    if(i2c_2_spi == 1) {
      if((i2c_resp=ioctl(i2c_fd, I2C_SLAVE_FORCE, ADDR_I2C2SPI)) >= 0) {
         i2c_write_addr_data16(i2c_fd, 2, data);
      }
      alex_data_tx = data;
    } else {
       // End of KA6S add
...
fprintf(stderr,"Starting Main\n");
  ...
  if((i2c_fd = open("/dev/i2c-0", O_RDWR)) >= 0)
  {
    ...
    // Detect KA6S system - set i2c_alexrx/i2c_alextx if valid
    if(ioctl(i2c_fd, I2C_SLAVE_FORCE, ADDR_I2C2SPI) >= 0) {
      if(i2c_write_addr_data16(i2c_fd, 0x01, 0x0) > 0) {
         #ifdef DEBUG         
         fprintf(stderr,"I2C2SPI\n");
         #endif
         i2c_2_spi = 1;
      }
    }
